...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	//Right Arrow increases the image number
	else if(key == GLFW_KEY_RIGHT && action == GLFW_PRESS)
	{
		origin[0] += 0.1f;
	}
	//Left Arrow decreases the image number
...
int main(int argc, char *argv[])
{
	...
	glUseProgram(shader.program);
	GLint myObjectsUniform = glGetUniformLocation(shader.program, "myObjects");

	glUseProgram(shader.program);
	GLint arraySizeUniform = glGetUniformLocation(shader.program, "arraySize");

	glUseProgram(shader.program);
	GLint originUniform = glGetUniformLocation(shader.program, "origin");
	...
	//#endif
	{
		...
		glUseProgram(shader.program);
		glUniform1fv(myObjectsUniform, sizeof(float) * arraySize, &myObjects[0]);

		glUseProgram(shader.program);
		glUniform1i(arraySizeUniform, arraySize);

		glUseProgram(shader.program);
		glUniform3f(originUniform, origin[0], origin[1], origin[2]);
...
void parseScene(string fileName){
	...
	while( getline( file, line ) )   
	{
		...
		{
			myObjects.push_back(::atof(result.c_str()));
			arraySize++;
		}
	}
